...
基于2024年行业最佳实践和技术趋势，本项目采用以下技术栈：

### 后端技术栈
- **编程语言**: Python 3.11+ (AI生态最成熟)
- **Web框架**: FastAPI (高性能异步框架，原生支持OpenAPI)
- **数据库**: PostgreSQL 15+ (主数据库) + Redis 7+ (缓存/会话)
- **ORM**: SQLAlchemy 2.0 + Alembic (数据库迁移)
- **数据验证**: Pydantic v2 (类型安全和数据验证)

### AI/ML技术栈
- **LLM服务**: OpenAI兼容API (支持Qwen2.5-72B-Instruct, GLM-4, DeepSeek等中文模型)
- **Function Calling**: 支持OpenAI Function Calling标准，实现CRM系统操作
- **MCP协议**: Model Context Protocol支持，实现外部工具和数据源集成
- **Agent框架**: LangGraph (状态管理) + LangChain (LLM编排)
- **向量数据库**: Qdrant (开源高性能向量数据库)
- **嵌入模型**: BGE-M3 (BAAI出品，多语言嵌入模型，中英文效果业界领先)
- **文档处理**: LlamaIndex (文档索引和检索) + 中文分词优化
- **重排序**: BGE-reranker-v2-m3 (BAAI出品，中文重排序效果最佳，显著提升检索精度)

### 前端技术栈
- **框架**: React 18 + TypeScript + Vite
- **UI库**: Shadcn/ui + Tailwind CSS (现代设计系统)
...
- **API网关**: Kong (统一入口和路由)
- **消息队列**: RabbitMQ (Agent间通信) + Apache Kafka (事件流)
- **监控**: Prometheus + Grafana + AlertManager
- **日志**: ELK Stack (Elasticsearch + Logstash + Kibana)
- **部署**: GitOps (ArgoCD) + Helm Charts
